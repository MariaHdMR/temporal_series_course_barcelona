...
dat_daily_2019 = dat_2019 %>% 
  ...
  summarise(air.temp.mean = mean(air.temp, na.rm= TRUE),
            ...
            .by= "date") # el by aqui es como el group by. 
...
NOTA: esto lo que hace es ver s dos puntos de una misma serie temporal estan correlacionadas o no
...
ccf (dat_2019$air.temp, dat_2019$water.temp,
     ...
     plot = "TRUE")
#ACF es el coeficiente de correlacion
#
...
# esto lo que hace es sacarnos el punto máximo de la curva de distribucion y sacar el valor de la x que correspode con el valor mas alto.
...
# 1) water temperature with oxygen
ccf (dat_2019$water.O2, dat_2019$water.temp,
     #indicate what to do with "NA".
     na.action = na.pass,
     #indicate if you want the plot
     plot = "TRUE")
...
Find_Max_CCF (dat_2019$water.O2, dat_2019$water.temp)
# estan negativamente correlacionados y el lag time es de 0

# 2) water temperature with carbon dioxide

ccf (dat_2019$water.CO2, dat_2019$water.temp,
     #indicate what to do with "NA".
     na.action = na.pass,
     #indicate if you want the plot
     plot = "TRUE")
...
Find_Max_CCF (dat_2019$water.CO2, dat_2019$water.temp)
#estan correlacionados negativos y el lag es 39
...
NOTA: el indice de kuramato va de 0 a 1
------------------------------------------------------------------------
...
cor_TS = dat_soils %>%
          ...
          # if you have several sensors, it will save time to create a loop
          summarize(
            cor_s1_s2 = cor(TS1, TS2),
            cor_s1_s3 = cor(TS1, TS3),
            cor_s1_s4 = cor(TS1, TS4),
            cor_s2_s3 = cor(TS2, TS3),
            cor_s2_s4 = cor(TS2, TS4),
            cor_s3_s4 = cor(TS3, TS4)
            ) %>%
          # calculate the mean correlation coefficient
          mutate (mean_cor_TS = rowMeans(select(., starts_with("cor_s")))) %>%
          # select week and mean correlation coefficient
          select (week, mean_cor_TS)

cor_TS


cor <- full_join(cor_SWC, cor_TS)

ggplot(cor, aes(x=mean_cor_SWC, y = mean_cor_TS, color= week))+
  geom_point()+
...
acf(mod$residuals) # aqui vemos que hay autocorrelacion

library(DHARMa)

simulateResiduals(fittedModel = mod, plot = T) #aqui no vemos si hay autocorrelacion o no
...
-   **Generalized Least Squares (GLS) Models:** GLS is an extension of
    ...
    #NOTA: la gente de series temporales suele usar gls en vez de lme, que parece ser que funciona mejor
...
### NOTA: lo primero que tengo que hacer es hacer la media por ejemplo al mes. 
El siguiente paso sera hacer descompose
y el diguiente es hacer un model regresion. 
...
hacer: 
1) cross correlation
2) deal with annual data
3) point cero would be summer, porque es de lo que tienen datos de la biomasa
4) model regression de la lag time que hemos visto. 
